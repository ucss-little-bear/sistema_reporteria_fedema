...
class FirmarReportesGeneradosView extends StatefulWidget {
  ...
  State<FirmarReportesGeneradosView> createState() =>
      _FirmarReportesGeneradosViewState();
}

class _FirmarReportesGeneradosViewState
    extends State<FirmarReportesGeneradosView>
  ...
  void _distribuirDatos(List<Reporte> data) {
    ...
    _listaRendimiento.clear();
    _listaCertificados.clear();

    for (var r in data) {
      if (r.tipoReporte == 'Boleta de Notas') {
        String k = r.informacionAdicional ?? "Otros";
        if (!_boletasAgrupadas.containsKey(k)) _boletasAgrupadas[k] = [];
        _boletasAgrupadas[k]!.add(r);
      } else if (r.tipoReporte == 'Reporte de Rendimiento') {
        _listaRendimiento.add(r);
      } else {
        _listaCertificados.add(r);
      }
    }
  }
                ...
                child: Column(
                  children: [
                    Row(
                      ...
                      children: [
                        const Text(
                          "Vista Previa (Solo Lectura - No Exportable)",
                          style: TextStyle(
    ...
    bool ok =
        await showDialog<bool>(
          ...
          builder: (c) => AlertDialog(
            ...
            content: const Text(
              "Al firmar, se estamparán sus credenciales (Nombre, DNI, Fecha) en el documento.\n¿Desea proceder?",
            ),
            actions: [
              TextButton(
    ...
    bool ok =
        await showDialog<bool>(
          ...
          builder: (c) => AlertDialog(
            ...
            content: Text(
              "Está a punto de firmar digitalmente ${lista.length} boletas del salón $salon.\nSe aplicará su estampa de firma a todos los documentos.",
            ),
            actions: [
              TextButton(
                onPressed: () => Navigator.pop(c, false),
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            "Revise y aplique su firma digital a los documentos generados.",
          ...
          Container(
            ...
            child: TabBar(
              ...
              tabs: [
                _tab("Boletas", _countBoletas(), Icons.folder_shared),
                _tab("Rendimiento", _listaRendimiento.length, Icons.bar_chart),
                _tab(
                  "Certificados",
                  _listaCertificados.length,
  ...
  Widget _buildListView(List<Reporte> list, IconData icon, Color color) {
    if (list.isEmpty) return _empty("No hay documentos pendientes.");
    return ListView.builder(
      itemCount: list.length,
      itemBuilder: (c, i) => _cardItem(list[i], icon, color),
    );
  }
  ...
  Widget _cardBoletaGroup(String key, List<Reporte> list) {
    ...
    String docente = p.length > 5 ? p[5] : "Docente Asignado";
    ...
    return Card(
      ...
      child: ExpansionTile(
        leading: Container(
          ...
          child: const Icon(Icons.folder_shared, color: Colors.purple),
        ),
